...
class Environment:
    ...
    def get_neighbors(self, position):
        """Returns the valid neighboring positions (avoiding barriers)."""
        x, y = position
        neighbors = [(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)]
        return [(nx, ny) for nx, ny in neighbors if self.is_within_bounds(nx, ny) and not self.is_barrier(nx, ny)]
